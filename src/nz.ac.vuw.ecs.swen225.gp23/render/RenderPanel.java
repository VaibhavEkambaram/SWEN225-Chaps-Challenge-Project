...
public class RenderPanel extends JPanel {
   ...
   *
   * @param rows    Number of rows for the display grid.
   * @param cols    Number of cols for the display grid.
   ...
   *
   * @param currentBoard Next board state.
   * @author Cameron Li - 300490702
   */
  ...
  private void createGrid() {
    ...
    for (int row = 0; row < rows; row++) { // Create JLabels for each tile
  ...
  public void paint(Graphics g) {
    ...
    if (!isPaused) {
      ...
      for (int row = 0; row < rows; row++) {
        for (int col = 0; col < cols; col++) {
          ImageIcon foundTile =
              TileFinder.getTile(displayBoard[row][col], tileset);
          Image resizeImage =
              foundTile.getImage().getScaledInstance(size, size, Image.SCALE_SMOOTH);
